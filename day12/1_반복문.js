/*
    조건문 : 특정한 조건의 논리(true/false) 결과에 따라서 명령어들을 처리한다.
        if
    반복문 : 특정한 조건의 논리 결과에 ture 일때 특정한 명령어들을 반복 처리한다. 
        for
*/
...
console.log(3);
console.log(4);
console.log(5);     // 반복되는 코드를 복사/붙여넣기를 하면된다. 다른 부분 일부 수정 
    // *문제점 : 반복되는 코드수가 많으면 (관리/코드수증가)힘들다. 100000번
...
총합계 = 총합계 + 5;
// 반복되는 코드        : 총합계 = 총합계 + <?>
// 반복되지 않은 코드   : let 총합계 = 0; : 패턴없음 , 1 2 3 4 5 : 패턴존재(1부터 5까지 1씩증가)
    // (반복문)
let sum = 0;
for( let 더할값 = 1; 더할값 <= 5; 더할값++ ){
    // 더할값을 1부터 5이하까지 더할값 1씩 증가 하면서{}를 반복 하겠다.
    sum = sum + 더할값;
}
...
// 주의할점 : 배열에 입력받은 값을 push 했을때 num1,num2,num3 필요한가?  필요(X)
