...
/*
    ...
    4. (데이터모델링) : 표로 표현된 자료들을 js 데이터 표현식
            1. 테이블내 행(가로) 1개가 객체1개
            2. 행(객체) 모여서 테이블(배열) 1개
    5. (로직)
            1. 현재 회원들 중에서  구독중인 회원찾기
                (1) 회원테이블(배열) 조회하여 하나씩 회원정보 확인
                (2) 하나씩 조회중인 회원정보가 구독로그테이블(배열) 내 존재하는지 확인
                (3) 일치한 회원코드가 있으면(구독중인상태) '구독중'
                    일치한 회원코드가 없으면(구독중이아닌상태)  '구독 내역이 없습니다.' 를 document.write 로 출력하시오.
...
for( let i = 0; i <= 회원테이블.length-1; i++){
    ...
    for( let j = 0; j <= 구독로그테이블.length-1; j++){
        let 구독로그 = 구독로그테이블[j];
        if(구독로그.회원코드 == 회원.회원코드){ // * 만약에 i번째 회원코드가 j번째 구독회원코드 와 같으면
            구독상태 = true;    // 현재가 회원코드가 구독로그에 존재한다.!! (체크!!)
            구독상품코드 = 구독로그.상품코드;   // 현재 구독중인 상품코드를 기록한다.
            // console.log(회원테이블[i]);
            if( today > 구독로그.구독종료일 ){  // 만약에 구독중인 구독종료일이 현재날짜 보다 작으면    
                구독활성화 = '만료'; console.log('만료');              
            }else{ 구독활성화 = '활성'; console.log('활성'); }                      
        } // * 구독 로그를 전체 확인 ( 반복문이 종료될때까지 확인 )
    } // for2 end     
    let html = `<h3>${회원.회원이름}</h3>`
    if ( 구독상태 == true ){ // 구독중
        for( let a = 0; a <= 구독상품테이블.length-1; a++){
            let 구독상품 = 구독상품테이블[a]
            if( 구독상품.상품코드 == 구독상품코드 ){    // a번째 구독상품코드 와 i번째 회원의 구독중인 상품코드 와 같으면
                html += `<ul><li> 구독상품 : ${구독상품.상품명} (${구독상품.가격})</li><li> 상태 : ${구독활성화}</li></ul>`
            }
            // console.log(구독상품)
