...
// let temp = 0
// for( let i = 0; i < numbers.length-1; i++){
//     for( let q = i+1; q <= numbers.length-1; q++ ){
//         if( numbers[q] > numbers[i] ){             
//             temp = numbers[q]
//             numbers[q] = numbers[i]
//             numbers[i] = temp            
//         }
...
// let stock = [10, 5, 20];
// let 상품명 = prompt(' 상품명입력');// (1) 사용자로부터 구매할 상품명 입력받아
// let 수량 = Number(prompt(''));      // (2) 사용자로부터 구매할 수량을 입력받아
// 재고가 충분( 비교/논리 판단 ) , 재고를 차감 ( 연산/수정 )
// (2) 제품목록에서 입력받은 값 찾아라 , (2-1) 제품목록의 모든 제품들을 순회(조회)
// for( let i = 0; i <= products.length-1; i++ ){
//     let product = products[i]; // i 번째의 제품 1개 호출
//     if( product == 상품명 ){ // 만약에 i번쨰 제품명과 입력받은 제품명과 같으면
//         console.log( '존재하는 재품명');
//         if( stock[i] >= 수량 ){ // 만약에 i번째 재고가 입력받은 수량보다 더크면
//             stock[i] -= 수량; // i 번째에 재고를 입력받은 수량만큼 차감(빼기) 한다.
//             console.log( '구매 완료!' );
//         }else{ // 아니면
//             console.log( '제고가 부족합니다.' );
//         } // if2 end
//     } // if1 end
// } // for end
...
//     let seat = seatStatus[i]; // i번째의 좌석 호출
//     if( seat == "빈좌석" ){
