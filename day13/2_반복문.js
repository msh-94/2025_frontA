...
//     output += `<div> ${name}`;// (1) HTML 문자열 추가
//     // 점수 출력
//     let score = scores[i];
//     let black = parseInt( score / 10 ); // 92/10 -> 9.2 -> parseInt(9.2) -> 9
//     for( let circle = 1; circle <= 10 ; circle++){  // 동그라미는 1부터 10이하까지 1씩증가 ( 총 10번 )
//         if( circle <= black ){ output+= ` <span>●</span>`}  // 만일 동그라미가 점수보다 작으면 검은
