...
// localStorage.setItem( 'userSettings' , JSON.stringify( { theme : 'dark' , notifications : true , language : 'ko' } ) );
// 1. 객체를 문자열로 변환
...
// 1. 기존의 웹스토리지 불러오기/확인
let visit = sessionStorage.getItem('visit');          console.log(visit); // 존재하지 않은 속성명은 null , 존재하는 속성명은 값 반환
if( visit == null ){ // 속성명이 존재하지 않을때 속성 초기화 해주기
    // 2. 새로운 속성 만들기
    sessionStorage.setItem('visit' , 1 );
    // *
    visit = 1; 
}else{ // 이미 존재하는 속성명 이면 값 호출
