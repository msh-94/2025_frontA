...
/*
let 정수 = Number(prompt("정수를 입력해주세요."));
let 결과 = 정수 % 7 == 0
console.log( `결과 : ${ 결과 }`);  
*/
...
let 비밀번호 = Number(prompt("비밀번호 입력해주세요."));
let IDN = 아이디 == 'admin' && 비밀번호 == 1234;
console.log( `최종 : ${IDN}`) ;
*/
// [지문7] prompt 함수로 정수를 입력받아 입력받은 값이 홀수 이면서 7배수 이면 true / 아니면 false 로 console탭에 출력하시오.
/*
let 정수 = Number(prompt("정수를 입력해주세요."));
let 결과 = ( 정수 % 7 == 0 ) && ( 정수 % 2 == 1 );
console.log( `결과 : ${결과}`);
*/
...
console.log( 나이++ );  // 41++ --> 42[x] 41[o] 
    ...
    console.log( 나이 );    42
    --나이; // 증강연산자 외 다른 명령어가 없으면 선위/후위 가 상관없다. * 세미콜론 단위 우선순위 따짐
    나이--; 
...
let 점수1 = prompt("1차점수 :");                // 변수명은 숫자로 시작 불가능
let 점수2 = prompt("2차점수 :");                
let 총합 = Number(점수1) + Number(점수2);       // prompt 는 항상 문자열로 반환/결과 준다.
let 총점 = 총합 >= 150 ? "합격" : "불합격";     // 논리는 항상 true 와 false로 답한다, 삼항 또는 조건문
console.log(` 결과 : ${총점}`);                 // true 대신에 '합격' , false 대신에 '불합격'
// [지문9] 두 사람의 이름을 prompt함수로 각 입력받아서 만일 이름이 '유재석' 이면 뒤에 (방장) 이라고 이름 뒤에 붙이고 아니면 생략한다.  HTML의 <ol> 에 결과를 출력하시오.
let 이름 = prompt("이름1을 입력해주세요.");
let 방장 = 이름 == "유재석" ? `${이름}(방장)` : `${이름}`;   // * 변수명은 중복이 불가능하다.*
                // 조건      ?  참              :   거짓
document.write(`<ol><li> ${방장}</li></ol>`)