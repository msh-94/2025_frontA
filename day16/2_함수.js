...
function func2( x , y ){ console.log( 
    `fun2 exe ${ x+y } `); }
...
func4( );               // 매개변수x , 반환값o(변수에 저장 o/x 선택 )
...
if( true ){
    ...
    // console.log( 지역변수2 ); // fail 오류발생 부평구 : 현 위치는 인천광역시 라서 부평구 호출[x]
}
// console.log( 지역변수1 ); // fail 오류발생 : 현 위치에서 인천광역시 호출[x]
// console.log( 지역변수2 ); // fail 오류발생 : 현 위치에서 부평구 호출[x]
...
// 현 기준으로 변수는 총 몇개일까요? 1개 nameList
    // vs
let nameList2 = [];
let name1 = prompt(' 이름 : ');
nameList2.push( name1 );
let name2 = prompt(' 이름 : ');
nameList2.push( name2 );
// 현재 코드흐름이 65번째 일때 변수는 총 몇개일까요? 3개 nameList2 , name1 , name2
